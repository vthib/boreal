...
pub(crate) fn serialize_header<W: io::Write>(kind: [u8; 4], writer: &mut W) -> io::Result<()> {
    ...
    // Add size of usize in the header: if it does not match,
    // deserialization cannot work.
    #[allow(clippy::cast_possible_truncation)]
    (size_of::<usize>() as u8).serialize(writer)?;
    Ok(())
}
...
pub(super) fn deserialize_header<R: io::Read>(
    ...
    let ptr_size: u8 = BD::deserialize_reader(reader)?;
    if usize::from(ptr_size) != size_of::<usize>() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "pointer size used during serialization does not match host: {} != {}",
                ptr_size,
                size_of::<usize>(),
            ),
        ));
