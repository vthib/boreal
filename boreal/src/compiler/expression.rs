...
#[derive(Debug)]
pub(super) enum BoundedIdentifierType {
    /// Type generated by a module use.
    Module(ModuleType),
    /// Type generated by an expression.
    Type(Type),
}
...
pub enum Expression {
    ...
    /// A bounded identifier from a for expression.
    ///
    ...
    match iterator {
        parser::ForIterator::Identifier(identifier) => {
            ...
            match iterator_type {
                module::IteratorType::Array(value_type) => {
                    match &identifiers {
                        &[name] => compiler.add_bounded_identifier(
                            name,
                            BoundedIdentifierType::Module(value_type),
                            identifiers_span,
                        )?,
                        _ => invalid_binding(1)?,
                    };
        ...
        parser::ForIterator::Range { from, to } => {
            ...
            match &identifiers {
                &[name] => compiler.add_bounded_identifier(
                    name,
                    BoundedIdentifierType::Type(Type::Integer),
                    identifiers_span,
                )?,
                _ => invalid_binding(1)?,
            };
        ...
        match identifier_type {
            ...
            BoundedIdentifierType::Type(typ) => {
                if identifier.operations.is_empty() {
                    Ok((Expression::BoundedIdentifier(*index), *typ))
                } else {
